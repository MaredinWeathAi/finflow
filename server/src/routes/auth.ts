...
router.post('/seed-demo', (req: Request, res: Response) => {
  try {
    ...
    const insertRecurring = db.prepare(`INSERT INTO recurring_expenses (id, user_id, account_id, name, amount, category_id, frequency, next_date, last_charged_date, is_active, notes, price_history, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,1,?,?,?,?)`);
    insertRecurring.run(uuid(), uid, checkingId, 'Rent', 2200, cats['Housing'], 'monthly', '2026-04-01', '2026-03-01', null, '[]', now, now);
    insertRecurring.run(uuid(), uid, creditId, 'Netflix', 15.99, cats['Subscriptions'], 'monthly', '2026-04-05', '2026-03-05', null, '[]', now, now);
    insertRecurring.run(uuid(), uid, creditId, 'Spotify', 10.99, cats['Subscriptions'], 'monthly', '2026-04-07', '2026-03-07', null, '[]', now, now);
    insertRecurring.run(uuid(), uid, creditId, 'Planet Fitness', 24.99, cats['Subscriptions'], 'monthly', '2026-04-15', '2026-03-15', null, '[]', now, now);
    insertRecurring.run(uuid(), uid, creditId, 'ChatGPT Plus', 20.00, cats['Subscriptions'], 'monthly', '2026-04-18', '2026-03-18', null, '[]', now, now);
    insertRecurring.run(uuid(), uid, checkingId, 'Verizon FiOS', 89.99, cats['Utilities'], 'monthly', '2026-04-20', '2026-03-20', null, '[]', now, now);
    insertRecurring.run(uuid(), uid, checkingId, 'State Farm Insurance', 178.50, cats['Insurance'], 'monthly', '2026-04-03', '2026-03-03', null, '[]', now, now);
