...
export function ReportsPage() {
  ...
  const handleExportJSON = async () => {
    try {
      const data = await api.get<any>('/data/export')
      downloadFile(JSON.stringify(data, null, 2), 'finflow-data.json', 'application/json')
  ...
  return (
    <div>
      ...
      {/* Export Section */}
      <div className="mt-8">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">Export Data</h3>
        <div className="grid sm:grid-cols-3 gap-3">
          <button onClick={handleExportCSV} className="flex items-center gap-3 bg-card rounded-xl border border-border/50 p-4 hover:bg-accent/30 transition-colors text-left">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Download transactions as CSV</p>
            </div>
          </button>
          <button onClick={handleExportJSON} className="flex items-center gap-3 bg-card rounded-xl border border-border/50 p-4 hover:bg-accent/30 transition-colors text-left">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Full data backup as JSON</p>
            </div>
          </button>
          <button
            onClick={() => window.print()}
            className="flex items-center gap-3 bg-card rounded-xl border border-border/50 p-4 hover:bg-accent transition-colors text-left w-full"
          >
            <Download className="w-8 h-8 text-muted-foreground" />
            <div>
              <p className="text-sm font-medium">Print / Save PDF</p>
